...
$regional = mysqli_query($conn, "SELECT * FROM login WHERE id = '$string_id'");
$nama_regional = mysqli_fetch_array($regional);
$field_regional = $nama_regional["Regional"];
$user = $nama_regional["username"];
$approval_position = $nama_regional["ApproveLevel"];
...
$total_data = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM insert_voucher WHERE tahap_approve3 = '0' AND tahap_approve2 = '1' AND tahap_approve1 = '1' AND Nama_Cabang IN (SELECT id FROM cabang_master WHERE Regional = '$field_regional' AND nomorVoucher LIKE '%$cari%'"));
$total_page = ceil($total_data/$per_page); //round up value
$offset = ($cur_page - 1) * $per_page;
// END OF PAGINATION

$post = mysqli_query($conn, "SELECT id, tanggal, last_edited, nomorVoucher, Nama_Cabang, 
														 ...
														 FROM insert_voucher WHERE tahap_approve3 = '0' AND tahap_approve2 = '1'
														 AND tahap_approve1 = '1' AND 
														 Nama_Cabang IN (SELECT id FROM cabang_master WHERE Regional = '$field_regional') AND nomorVoucher LIKE '%$cari%' ORDER BY id DESC
														 LIMIT $per_page OFFSET $offset");

// post untuk list dropdown
$dropdown_post = mysqli_query($conn, "SELECT * FROM cabang_master WHERE Regional = '$field_regional' ORDER BY id ASC");
		...
		<div class="container">
			...
			<div class="row">
		            ...
		            <nav class="navbar navbar-expand-lg navbar-light">
		              ...
		              <div class="collapse navbar-collapse" id="navbarNavDropdown">
		                <ul class="navbar-nav mr-auto">
		                	<li class="nav-item">
		                    <a class="nav-link" href="dashboardRG.php">Dashboard</a>
		                  </li>
		                  <li class="nav-item">
		                    <a class="nav-link" href="directoryRG.php">Pending</a>
		                  </li>
		                  <li class="nav-item">
		                    <a class="nav-link" href="ApprovedRG.php">Approved</a>
		                  </li>
		                </ul>
		      ...
		      <div class="row">
			      ...
			      <a href="directoryRG.php" class="btn btn-outline-success my-2 my-sm-0">Cancel</a>
			    </form>
				</div>
			...
			<a href="directoryRG.php" class="badge badge-info mx-1 my-1">All</a>

    		<?php if(mysqli_num_rows($dropdown_post)) {?>
    		<?php while($row_dropdown_post = mysqli_fetch_array($dropdown_post)) {?>

    			<a href="directoryRG_filter.php?Filter=<?php echo $row_dropdown_post["id"]?>" class="badge badge-info mx-1 my-1"><?php echo $row_dropdown_post["id"]?></a>
					  ...
					  <tbody>
					  	...
					  	<?php if(mysqli_num_rows($post)) {?>
						    ...
						    <tr>
						      ...
						      <td>
						      	<a href="previewRG.php?previews=<?php echo $row_post["id"]?>" class="btn btn-primary mb-1">Preview</a>
						      	<a href="edit_RG.php?edits=<?php echo $row_post["id"]?>" class="btn btn-primary mb-1">Edit</a>
						      	<a href="delete_RG.php?deletes=<?php echo $row_post["id"]?>" class="btn btn-primary mb-1">Delete</a>
						      </td>
						...
						<?php if($total_page > 1) {?>
							<nav aria-label="Page navigation example text-center">
							  <ul class="pagination">
							  	<?php if($cur_page > 1) {?>
								    <li class="page-item">
								    	<a href="directoryRG.php?page=<?php echo $cur_page - 1?>" class="page-link" aria-label="Previous">
								...
								<?php if($cur_page < $total_page) {?>
								    <li class="page-item">
								    	<a href="directoryRG.php?page=<?php echo $cur_page + 1?>" class="page-link" aria-label="Next">
