...
$total_data = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM insert_voucher WHERE tahap_approve4 = '1' AND nomorVoucher LIKE '%$cari%'"));
...
$post = mysqli_query($conn, "SELECT id, tanggal, last_edited, nomorVoucher, Nama_Cabang,
														 ...
														 FROM insert_voucher WHERE tahap_approve4 = '1' AND nomorVoucher LIKE '%$cari%' ORDER BY id DESC
		...
		<div class="container">
			...
			<div class="row">
		      ...
		      <div class="row">
			      ...
			      <a href="ApprovedH1.php" class="btn btn-outline-success my-2 my-sm-0">Cancel</a>
			    </form>
		      </div>
		      ...
		      <div class="row">
			    ...
			    <label class="mx-1 my-1">Filter Cabang: </label>
					<a href="ApprovedH1.php" class="badge badge-info mx-1 my-1">All</a>

		    		<?php if(mysqli_num_rows($dropdown_post)) {?>
		    		<?php while($row_dropdown_post = mysqli_fetch_array($dropdown_post)) {?>

		    			<a href="ApprovedH1_filter.php?Filter=<?php echo $row_dropdown_post["id"]?>" class="badge badge-info mx-1 my-1"><?php echo $row_dropdown_post["id"]?></a>
					  ...
					  <tbody>
					  	...
					  	<?php if(mysqli_num_rows($post)) {?>
						    ...
						    <tr>
						      ...
						      <td>
						      	<a href="previewH1_approved.php?previews=<?php echo $row_post["id"]?>" class="btn btn-primary">Preview</a>
						      </td>
						...
						<?php if($total_page > 1) {?>
							<nav aria-label="Page navigation example text-center">
							  <ul class="pagination">
							  	<?php if($cur_page > 1) {?>
								    <li class="page-item">
								    	<a href="ApprovedH1.php?page=<?php echo $cur_page - 1?>" class="page-link" aria-label="Previous">
								...
								<?php if($cur_page < $total_page) {?>
								    <li class="page-item">
								    	<a href="ApprovedH1.php?page=<?php echo $cur_page + 1?>" class="page-link" aria-label="Next">
