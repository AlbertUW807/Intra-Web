...
$filter = mysqli_query($conn, "SELECT * FROM login WHERE ApproveLevel = '$filter_id'");
$row_filter = mysqli_fetch_array($filter);
$field_filter = $row_filter["ApproveLevel"];
...
$total_data = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM login WHERE ApproveLevel = '$field_filter'"));
$total_page = ceil($total_data/$per_page); //round up value
$offset = ($cur_page - 1) * $per_page;
// END OF PAGINATION

$post = mysqli_query($conn, "SELECT * FROM login WHERE ApproveLevel = '$field_filter' ORDER BY id DESC
		...
		<div class="container">
			...
			<div class="row">
		      ...
		      <div class="alert alert-info mx-1 my-1" role="alert">
				  Levels : (A1 - Treasury) , (F1 - BFM) , (RG - Regional) , (H1 - SPV_HO) , (H2 - Manager_HO) , (D1 - Directur), (L1 - Acc.)
				</div>
					  ...
					  <tbody>
					  	...
					  	<?php if(mysqli_num_rows($post)) {?>
						      ...
						      <td>
						      	...
						      	<a href="delete_D1.php?deletes=<?php echo $row_post["id"]?>" class="btn btn-primary">Delete</a>
						      </td>
						    </tr>
							<?php $counter++; }?>
						<?php }?>
						...
						<?php if($total_page > 1) {?>
							<nav aria-label="Page navigation example text-center">
							  <ul class="pagination">
							  	<?php if($cur_page > 1) {?>
								    <li class="page-item">
								    	<a href="userD1_filter.php?page=<?php echo ($cur_page - 1)."&Filter=".$filter_id?>" class="page-link" aria-label="Previous">
								...
								<?php if($cur_page < $total_page) {?>
								    <li class="page-item">
								    	<a href="userD1_filter.php?page=<?php echo ($cur_page + 1)."&Filter=".$filter_id?>" class="page-link" aria-label="Next">
