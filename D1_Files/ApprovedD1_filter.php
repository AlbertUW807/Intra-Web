...
$total_data = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM insert_voucher WHERE tahap_approve6 = '1' AND Nama_Cabang = '$field_filter'"));
...
$post = mysqli_query($conn, "SELECT id, tanggal, nomorVoucher, Nama_Cabang, case WHEN tahap_approve1 = '0' THEN 'Belum'
														 ...
														 FROM insert_voucher WHERE tahap_approve6 = '1' AND Nama_Cabang = '$field_filter' ORDER BY id DESC
		...
		<div class="container">
			...
			<div class="row">
		      ...
		      <div class="row">
				...
				<a href="ApprovedD1.php" class="badge badge-info mx-1 my-1">All</a>

	    		<?php if(mysqli_num_rows($dropdown_post)) {?>
	    		<?php while($row_dropdown_post = mysqli_fetch_array($dropdown_post)) {?>

	    			<a href="ApprovedD1_filter.php?Filter=<?php echo $row_dropdown_post["id"]?>" class="badge badge-info mx-1 my-1"><?php echo $row_dropdown_post["id"]?></a>
					  ...
					  <tbody>
					  	...
					  	<?php if(mysqli_num_rows($post)) {?>
						    ...
						    <tr>
						      ...
						      <td>
						      	<a href="previewD1_approved.php?previews=<?php echo $row_post["id"]?>" class="btn btn-primary mb-1">Preview</a>
						      	<a href="deleteApproved_D1.php?deletes=?<?php echo $row_post["id"]?>" class="btn btn-primary mb-1">Delete</a>
						      </td>
						...
						<?php if($total_page > 1) {?>
							<nav aria-label="Page navigation example text-center">
							  <ul class="pagination">
							  	<?php if($cur_page > 1) {?>
								    <li class="page-item">
								    	<a href="ApprovedD1_filter.php?page=<?php echo ($cur_page - 1)."&Filter=".$filter_id?>" class="page-link" aria-label="Previous">
								...
								<?php if($cur_page < $total_page) {?>
								    <li class="page-item">
								    	<a href="ApprovedD1_filter.php?page=<?php echo ($cur_page + 1)."&Filter=".$filter_id?>" class="page-link" aria-label="Next">
